...
Un utilitaire utilisé en lignes de commande écrit en python uniquement pour vous
aider à réduire la taille de vos fichiers image.

Cette application est souhaitée purement en python, avec aucune dépendance autre
que Pillow, Afin d’assurer la compatibilité avec un grand nombre de systèmes,
comme iPhone iPad faisant tourner Pythonista 3. Si vous n’avez pas de
restrictions quant aux dépendances, vous serez probablement mieux servis par un
des nombreux systèmes d’optimisation d’images qui sont, eux, basés sur quelques
dépendances externes bien connues.
